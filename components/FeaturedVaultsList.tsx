    ...
    <div className="w-full max-w-2xl">
      <div className="flex flex-col gap-3">
        {vaults.map(v => (
          ...
          >
            <div>
              <p className="text-brand-cream font-medium">{v.name}</p>
              <p className="text-brand-light text-xs mt-0.5 font-mono">{v.address.slice(0, 10)}…</p>
            </div>
            <div className="flex gap-2">
              <span className="text-xs bg-brand-card text-brand-cream px-2 py-0.5 rounded">{v.protocol}</span>
              <span className="text-xs bg-brand-card text-brand-light px-2 py-0.5 rounded">
        ...
        {vaults.length === 0 && (
          <p className="text-brand-light/60 text-sm">{t.noVaults}</p>
        )}
      </div>
    </div>
  )
