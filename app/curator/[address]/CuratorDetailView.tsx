...
function morphoMarketUrl(marketId: string, chainId: number): string {
  const network = chainId === 8453 ? 'base' : chainId === 42161 ? 'arbitrum' : 'ethereum'
    ...
    <div className="min-h-screen bg-brand-bg text-brand-cream p-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-brand-card border border-brand-border rounded-lg p-4">
          <p className="text-sm text-brand-light">{t.realizedBadDebt}</p>
          <p className={`text-xl font-bold ${totalBadDebtUsd > 0 ? 'text-brand-olive' : 'text-green-800'}`}>
            {totalBadDebtUsd > 0 ? fmtUsd(totalBadDebtUsd) : t.none}
          </p>
          <p className="text-sm text-brand-light mt-0.5">{eventCount} event(s) across {affectedMarketCount} market(s)</p>
        </div>
        <div className="bg-brand-card border border-brand-border rounded-lg p-4">
          <p className="text-sm text-brand-light">{t.unrealizedBadDebt}</p>
          <p className={`text-xl font-bold ${unrealizedBadDebtUsd > 0 ? 'text-orange-700' : 'text-green-800'}`}>
            {unrealizedBadDebtUsd > 0 ? fmtUsd(unrealizedBadDebtUsd) : t.none}
          </p>
          <p className="text-sm text-brand-light mt-0.5">{t.stuckBorrows}</p>
        </div>
        <div className="bg-brand-card border border-brand-border rounded-lg p-4">
          <p className="text-sm text-brand-light">{t.historicalVaults}</p>
      ...
      {unrealizedBadDebtUsd > 0 && (
        <div className="bg-orange-50 border border-orange-300 rounded-lg p-4 mb-8">
          <h2 className="text-base font-semibold text-orange-800 mb-1">{t.unrealizedDetected}</h2>
          ...
          {unrealizedMarkets.length > 0 && (
            <div className="mt-3 space-y-1">
              {unrealizedMarkets.sort((a, b) => b.badDebtUsd - a.badDebtUsd).map(m => (
                <div key={m.marketId} className="flex items-center gap-2 text-sm">
                  <a
                  ...
                  <span className="text-brand-light">{CHAIN_LABELS[m.chainId] ?? `Chain ${m.chainId}`}</span>
                  <span className="text-orange-800 font-semibold">{fmtUsd(m.badDebtUsd)}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {events.length === 0 && unrealizedBadDebtUsd === 0 ? (
        <div className="bg-brand-card border border-brand-border rounded-lg p-8 text-center">
          <p className="text-green-800 text-lg font-semibold">{t.cleanRecord}</p>
        ...
        <div className="bg-brand-card border border-brand-border rounded-lg overflow-hidden">
          <div className="px-4 py-3 border-b border-brand-border">
            <h2 className="text-base font-semibold text-brand-cream">{t.badDebtEvents}</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-brand-border text-brand-light">
            ...
            <tbody>
              {events.map((evt: BadDebtEvent, i: number) => (
                <tr key={`${evt.marketId}-${i}`} className="border-b border-brand-border/50 hover:bg-brand-border/20">
                  <td className="px-4 py-2">
                    <a
                      href={morphoMarketUrl(evt.marketId, evt.chainId)}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-brand-cream hover:text-brand font-mono text-sm"
                    >
                  ...
                  <td className="px-4 py-2 text-right text-brand-olive font-semibold">
